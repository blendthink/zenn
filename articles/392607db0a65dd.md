...
今回、紹介するのは `3.6.0-1.0.pre` で導入された `--dart-define-from-file` を使う方法です。

:::message
`3.6.0-1.0.pre` は master ブランチにタグづけされているだけで、正式にリリースされていません。
そのため、途中で機能が変更される可能性があるためご注意ください。
:::
...
`--dart-define-from-file` は、まだ正式にリリースされていない機能です。
そのため、リポジトリから対象のソースコード（ `3.6.0-1.0.pre` ）をダウンロードする必要があります。
...
Flutter SDK のパスを設定して、`3.6.0-1.0.pre` が使用されるようにします。
